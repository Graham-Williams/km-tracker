from app import calculate_placements, format_line
from db import get_connection
from helpers import create_cup_with_scores, create_player
...
def test_placements_negative_line_scores():
    """Negative line scores should be handled correctly."""
    scores = [
        {"player_id": 1, "score": 10, "line": -20, "won_tiebreaker": False},
        {"player_id": 2, "score": 5, "line": 0, "won_tiebreaker": False},
        {"player_id": 3, "score": 20, "line": -30, "won_tiebreaker": False},
    ]
    result = calculate_placements(scores)
    # Line scores: -10, 5, -10
    assert result[0]["player_id"] == 2   # 5 is highest
    assert result[0]["placement"] == 1
    # Players 1 and 3 are tied at -10
    assert result[1]["placement"] == 2
    assert result[2]["placement"] == 2


def test_placements_single_player():
    scores = [
        {"player_id": 1, "score": 50, "line": 0, "won_tiebreaker": False},
    ]
    result = calculate_placements(scores)
    assert result[0]["placement"] == 1
...
def test_no_line_adjustment_4_player_cup(client):
    ...
    create_player(client, "Bob", has_line=True)
    create_player(client, "Carol", has_line=True)
    create_player(client, "Dave", has_line=True)
...
def test_no_line_adjustment_three_way_tie_with_one_tiebreaker(client):
    """3-way tie with one tiebreaker still leaves 2nd/3rd unresolved — skip adjustments."""
    create_player(client, "Alice", has_line=True)
    create_player(client, "Bob", has_line=True)
    create_player(client, "Carol", has_line=True)
    create_cup_with_scores(
        client, "2026-03-15T20:00", [(1, 80), (2, 80), (3, 80)],
        tiebreaker_ids=[1],
    )
    conn = get_connection()
    alice = conn.execute("SELECT line FROM players WHERE id = 1").fetchone()
    ...
    changes = conn.execute("SELECT * FROM line_changes WHERE cup_id = 1").fetchall()
    conn.close()
    # Alice is 1st, but Bob and Carol share 2nd — unresolved tie
    assert alice["line"] == 0
    assert bob["line"] == 0
    assert carol["line"] == 0
    assert len(changes) == 0
...
def test_no_line_adjustment_2nd_3rd_tie_no_tiebreaker(client):
    """2nd and 3rd place tie without tiebreaker — skip adjustments."""
    create_player(client, "Alice", has_line=True)
    create_player(client, "Bob", has_line=True)
    create_player(client, "Carol", has_line=True)
    create_cup_with_scores(client, "2026-03-15T20:00", [(1, 100), (2, 70), (3, 70)])
    conn = get_connection()
    alice = conn.execute("SELECT line FROM players WHERE id = 1").fetchone()
    bob = conn.execute("SELECT line FROM players WHERE id = 2").fetchone()
    carol = conn.execute("SELECT line FROM players WHERE id = 3").fetchone()
...
def test_line_adjustment_2nd_3rd_tie_with_tiebreaker(client):
    """2nd and 3rd place tie resolved by tiebreaker — adjustments apply."""
    create_player(client, "Alice", has_line=True)
    create_player(client, "Bob", has_line=True)
    create_player(client, "Carol", has_line=True)
    create_cup_with_scores(
        client, "2026-03-15T20:00", [(1, 100), (2, 70), (3, 70)],
        tiebreaker_ids=[2],
    )
    conn = get_connection()
    alice = conn.execute("SELECT line FROM players WHERE id = 1").fetchone()
    ...
    conn.close()
    assert alice["line"] == -3  # 1st
    assert bob["line"] == 0     # 2nd (won tiebreaker)
    assert carol["line"] == 3   # 3rd


def test_update_cup_does_not_apply_line_adjustments(client):
    """Editing a cup should not trigger line adjustments."""
    create_player(client, "Alice", has_line=True)
    create_player(client, "Bob", has_line=True)
    create_player(client, "Carol", has_line=True)
    # Create a 2-player cup (no adjustments)
    create_cup_with_scores(client, "2026-03-15T20:00", [(1, 100), (2, 80)])
    # Edit it to have 3 players — should NOT trigger adjustments
    client.post(
        "/cups/1/edit",
        data={
