...
- **Mixed cups ("Wii + Switch", `game_edition = 'mixed'`)** — 4 races in **BLOCKS**: races 1–`RACES_PER_BLOCK` (=2) on the console a **server-side coin flip picked at cup creation**, then 2 on the other. Exactly one console swap.
  ...
  - **Stale page across the swap:** the spin JSON now returns `"edition"`; the client reloads on mismatch with the page's `RACE_EDITION` rather than indexing a result into the wrong wheel.
  ...
  - **Score entry is unchanged** — one combined total per player; no `scores` schema change, no stats change. The complete page shows a "combined total" note and, entering race 3, a **photo reminder** (`#swap-reminder-modal` + a persistent `#second-console-banner`): photograph the standings *before* switching, because the second console scores from zero. **That photo is never stored or extracted** by the app.
  - **Photo extraction:** `_players_for_extraction` maps a mixed cup to `race_edition(cup, MAX_RACES)` (the second half's console — the results screen actually on display). Passing `'mixed'` through would drop `default_character_field`/`build_extraction_prompt` into their unknown-edition branches. **Caveat:** auto-filled values on a mixed cup are the second console's HALF totals, not the cup total — acceptable because the mapping panel never auto-submits, but worth a follow-up. The manual `/cups/new` path's `edition not in TRACK_SETS` check correctly rejects `'mixed'` — that's not a bug.
