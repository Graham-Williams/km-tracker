...
- **Two invariants (verified + tested):**
  ...
  - **Completion uses the LIVE roster.** `cup_session_complete` renders players from `cup_players`, and `apply_line_adjustments` keys off the submitted scores (`len(scores_data)`), not any cached count — so a 3→2 Wii cup skips the 3-player line adjustment and a 2→3 cup applies it; scores save for exactly the final roster.
- Tests: `tests/test_roster_editing.py` (add/remove happy paths, all rejects incl. duplicate/below-min/not-in-cup/garbage-id/oversized-id/non-in-progress, 3→2 & 2→3 completion, late-add-not-forfeited).
