...
- **`extraction.py`** owns the Claude call — model `claude-sonnet-4-6`, `client.messages.parse()` with a Pydantic `Standings(rows: [{position, character, points}])` schema, one image block (base64) + prompt. All anthropic/API failures are wrapped in `ExtractionError`. The dev script `scripts/validate_extraction.py <photo>` calls the **real** API with the same prompt+schema to sanity-check real phone photos (costs one API call; never run by tests).
- **Route `POST /extract-scores`** (JSON): `{image: <base64>, mime_type: image/jpeg|png}` plus either `cup_id` (live session — players/edition come from the cup) or `edition` + `player_ids` (manual `/cups/new` form, always `wii`). Response `{scores: {player_id: points}, ambiguous: [names], unmatched_players: [names], raw_rows: [...]}`. Hostile-input posture matches the rest of the app: bad base64/mime/oversize/ids → 4xx JSON, no key → 503, upstream API failure → 502, never a 500 (`tests/test_photo_extraction.py`).
- **Matching is server-side** (`match_standings_to_players` in `app.py`): rows are matched to players by their **per-edition default character** — `players.default_character_wii` / `default_character_switch` (nullable columns; pickers on player create/edit validated against the rosters). Casefold+strip comparison; exactly one player ↔ exactly one row fills a score; shared characters or duplicate rows → `ambiguous`; CPU rows (claimed by nobody) are ignored; players with no character or no matching row → `unmatched_players`.
