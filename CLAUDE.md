...
- **Lines are Wii-only:** the line handicap (`-3/0/+3` deltas, `line_changes`, `players.line`) applies **only to Wii cups**. Switch (`mk8dx`) cups are lineless. Enforced at the gate in `apply_line_adjustments()` — it reads `cups.game_edition` and returns early (no `line_changes`, no `players.line` change) unless the cup is `wii`, so mk8dx scores store `line=0`/`line_score=score`. The line-entry UI is also hidden per-edition in `cup_session_complete.html` and `cup_edit.html` via `{% set lines_on = cup.game_edition == 'wii' %}` (each row's `show_line = player.has_line and lines_on`; `data-has-line` reflects the gated value so the raw/line-score sync JS treats Switch rows as plain-points). History line badges (`cups.html`) need no gate — mk8dx cups simply have no `line_changes` rows. Direct `/cups` cups have no edition picker so default to `wii` (lines apply). Tests: `test_mk8dx_cup_stays_lineless`, `test_wii_cup_still_applies_lines`, `test_*_complete_page_*_line_inputs`.
