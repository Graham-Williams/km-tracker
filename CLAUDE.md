...
- **`extraction.py`** owns the Claude call — model `claude-sonnet-4-6`, `client.messages.parse()` with a Pydantic `Standings(rows: [{position, character, points, is_highlighted}])` schema, one image block (base64) + prompt. **Highlight-aware (2026-07):** the prompt tells the model the results screen visually highlights the human players' rows (player badges / brighter row background) vs CPUs, and to set `is_highlighted=true` for exactly those human rows (`false` for CPUs); it still returns ALL rows, just tagged. `is_highlighted` defaults `False` on the model so absent flags don't break parsing. All anthropic/API failures are wrapped in `ExtractionError`. The dev script `scripts/validate_extraction.py <photo>` calls the **real** API with the same prompt+schema to sanity-check real phone photos (costs one API call; never run by tests).
- **Route `POST /extract-scores`** (JSON): `{image: <base64>, mime_type: image/jpeg|png}` plus either `cup_id` (live session — players/edition come from the cup) or `edition` + `player_ids` (manual `/cups/new` form, always `wii`). Response `{scores: {player_id: points}, ambiguous: [names], unmatched_players: [names], raw_rows: [{position, character, points, is_highlighted}, ...]}` — each raw row now carries `is_highlighted` so the client can build the mapping dropdowns from highlighted (human) rows only. Hostile-input posture matches the rest of the app: bad base64/mime/oversize/ids → 4xx JSON, no key → 503, upstream API failure → 502, never a 500 (`tests/test_photo_extraction.py`).
- **Matching is server-side + highlight-aware** (`match_standings_to_players` in `app.py`): rows are matched to players by their **per-edition default character** — `players.default_character_wii` / `default_character_switch` (nullable columns; pickers on player create/edit validated against the rosters). **Only HIGHLIGHTED (human) rows are considered** — a CPU that happens to share a human's default character (e.g. a CPU Yoshi alongside Lizzy's Yoshi) is dropped so the real human matches cleanly, and a player whose character appears ONLY as a CPU is left blank (`unmatched`) rather than grabbing the CPU's score. **Robustness fallback:** if the model flagged ZERO rows highlighted (bad/old detection), it falls back to matching ALL rows so the feature never regresses below the pre-highlight behavior. Casefold+strip comparison; exactly one player ↔ exactly one row fills a score; genuinely shared characters among highlighted rows (2+ humans on one char) or duplicate rows → `ambiguous`; players with no character or no matching highlighted row → `unmatched_players`.
- **Mix-and-match review UI** (`static/js/photo_score.js` + the shared `#photo-score`/`.photo-mapping` block in both `cup_session_complete.html` and `cup_new.html`): after extraction, a panel renders one `<select>` per roster player whose options are the highlighted rows (`"P1 · Bowser — 41 pts"`) plus `— leave blank —`, pre-selected to the auto-matched row (reconstructed client-side from the already-filled score inputs). A row chosen by one player is disabled in every other dropdown (live-updated on change). Selecting fills that player's `.score-input` and dispatches the existing `input` event (so line-sync + placement recalc run exactly as when typed); blank clears it. A **count-mismatch warning** (`# highlighted rows ≠ roster size`) and an **unassigned-highlighted-row callout** show as banners. Panel styles live in `app.css` (`.photo-mapping*`, `.photo-map-*`). Still **never auto-submits**. Covered by `tests/e2e/test_photo_attach.py::test_mapping_panel_mix_and_match`. (Mid-cup roster add/remove from the panel is a deliberate future follow-up, NOT built here.)
