...
- **Mixed cups ("Wii + Switch", `game_edition = 'mixed'`)** — 4 races in **BLOCKS**: races 1–`RACES_PER_BLOCK` (=2) on the console a **server-side coin flip picked at cup creation**, then 2 on the other. Exactly one console swap.
  ...
  - **Two photos, one per console** (issue #70). `cup_photos.block` (nullable INTEGER) tags which block a photo belongs to; **NULL = "the cup's photo"** (every pure cup, every pre-feature row — no backfill). Tagged by **ORDINAL, never an edition string**: the console is derived via `block_edition(cup, block)` = `race_edition(cup, (block-1)*RACES_PER_BLOCK + 1)`, exactly like races, so a photo row can never disagree with `first_edition`.
    ...
    - **`GET /cups/<id>/photo/<block>`** serves the newest photo for that block (same ETag/Cache-Control/304 as `cup_photo`). **`GET /cups/<id>/photo` is deliberately unchanged** — newest of ANY block; the `/cups` thumbnail and `test_newest_photo_wins_when_multiple` pin that. `cup_photos` stays **append-only** (no UNIQUE): replacing = another INSERT, newest-row-per-block wins, and a wrong-block upload is fixed by re-uploading (the bad row is inert).
  ...
  - **Staging seed** (`scripts/seed_staging.py`) includes completed mixed cups in **both** console orders and parks the in-progress cup **exactly at the console swap**, so `break-staging` always has mixed data to hammer. Seeded mixed cups also carry a **per-block breakdown that adds up** (`split_blocks`) and a photo per block; the in-progress cup gets block 1's only (it's parked at the swap). Those photos are a **synthetic 1×1 JPEG** — this repo is public and a real standings photo is a picture of somebody's living room, so **never commit one** (tests and seeds use generated bytes).
