...
- **Sanity numbers** (real data, 2026-09-18, half-life 90, all cups): Wii +18.4 sd 10.0 (n=20) → +18; Switch −0.6 ± 3.5 (n=5) → **even** (inside the noise); Mixed +8.9 sd 9.0 → +9 (from 100 pairs). The sd here is the spec's population-style spread around the weighted mean, so it reads slightly tighter than the prototype's sample sd.
