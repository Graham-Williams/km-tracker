...
- **Mixed cups ("Wii + Switch", `game_edition = 'mixed'`)** — 4 races in **BLOCKS**: races 1–`RACES_PER_BLOCK` (=2) on the console a **server-side coin flip picked at cup creation**, then 2 on the other. Exactly one console swap.
  ...
  - **Score entry is PER CONSOLE BLOCK** (issue #70, 2026-08). Each console's results screen starts from zero, so a mixed cup has TWO screens whose points must be added. The completion form collects them separately — `block1_scores[]` + `block2_scores[]`, labelled with the **real console names in played order** (`block_labels` from `block_edition(cup, n)`) — and the `scores[]` **total is CALCULATED**, written by JS as `b1 + b2` and marked **`readonly`**. It must be `readonly` and **never `disabled`**: a disabled input isn't submitted, so `player_ids[]`/`scores[]` counts would diverge and the issue-#45 misalignment guard would refuse the whole form. `.score-input` stays the class on the total so `getScoreGroups`/`updatePlacements`/`updateTiebreakers` are untouched; the block listener dispatches a real `input` event so placements/tiebreakers recompute as if typed.
    - **Schema:** `scores.block1_score` / `block2_score` (nullable) alongside the untouched `scores.score` (the cup TOTAL). Columns live on `scores`, **not a side table**, because `save_scores` is DELETE + re-INSERT — so every write path that doesn't know about blocks (manual `/cups`, `/cups/<id>/edit`) writes NULL automatically and `score == block1 + block2` holds **by construction**. A side table would survive those rewrites and silently disagree. `update_score` (`/scores/<id>/edit`) explicitly sets both to NULL: that form only knows the total, and a breakdown that lies about the number beside it is worse than none.
    - **Server is authoritative on the sum.** `parse_block_scores_from_form(form, scores_data)` (called from `cup_session_submit` **before** `zero_lines_if_lineless`/`validate_scores`, so ties are checked against the real total): both halves blank → row untouched; both present → set them **and overwrite the submitted total with their sum** (a stale/crafted total must never win); **exactly one present → `InvalidInput`** — that shape can't satisfy the invariant, and guessing the missing half is the exact failure class this feature exists to prevent. It reads the **RAW form arrays**, not the filtered `scores_data`, because `parse_scores_from_form` SKIPS rows with an empty `scores[]` — which is precisely what a no-JS blocks-only row looks like; those rows are INSERTed in form order via a cursor walked in lockstep (not keyed on `player_id`, which a duplicate-id submit would make ambiguous). `clear_blocks_if_not_mixed(cup, scores_data)` mirrors `zero_lines_if_lineless` as the authoritative guard: a **pure cup stores NULL blocks no matter what was posted**, and the route only *calls* the parser for mixed cups, so a pure cup's submit is byte-identical to before.
    - **The client mirrors the one-half rejection, because the server's rejection COSTS THE USER THE FORM.** A rejected completion is a flash + **redirect**, and the cup is still `in_progress` so `existing_scores` is `{}` — every number typed is gone. The old submit guard only asked "is any total non-empty?", which a second player's completed row satisfies, so one mistyped half wiped the whole form. The guard in `cup_session_complete.html` now also scans each `.score-row` for a partially filled breakdown and blocks the submit via `#submit-warning`. **The server stays authoritative — don't weaken it.** The message lives in ONE place, `app.BLOCK_HALF_MISSING_MSG`, injected into the template (`block_half_missing_msg|tojson`) so the two wordings can't drift; it says **"use 0 if they scored nothing on that console"** rather than the old "leave both blank and enter the total", which the UI cannot do (a mixed cup's total input is `readonly`). The realistic case is a player who joined mid-cup.
    - **An unchanged total KEEPS its breakdown (`preserve_block_scores`, exploratory-QA fix).** "A rewrite NULLs the blocks" was too broad: `/cups/<id>/edit` renders no block inputs and always resubmits the totals it displayed, so a **notes-only** edit silently destroyed a mixed cup's entire breakdown — and the completion page then showed blank halves above a **readonly** total, unrecoverable without retyping both consoles. `update_cup` now calls `preserve_block_scores(conn, cup_id, scores_data)` before `save_scores`: for each row the caller hasn't already resolved, it re-attaches the row's **own** stored halves **only when `block1 + block2` still equals the total being written**. So the invariant is what's enforced, not the mechanism — a row whose total actually moved still goes NULL, sibling rows are independent, pure cups (no stored halves) are unaffected, and `/scores/<id>/edit` still NULLs its own row only. **Don't "simplify" this back to an unconditional wipe.**
    - **The client's total math must match what the server will ACCEPT.** `syncBlockTotal` used `parseInt`, which truncates (`"1e3"` → 1, `"2.5"` → 2) and so displayed a confident total for input the server rejects — and a rejection redirects, discarding the whole form. Halves now go through `blockValue()` (`valueAsNumber` + `Number.isInteger` + a digits-only test); anything else counts as "not filled in", and the submit guard blocks a typed-but-not-whole row with its own message. The inputs carry `step="1"`.
    - **Mixed cups are still LINELESS** — no line handling was added, and none should be.
    - **Phone layout (`.score-row--mixed`, exploratory-QA fix).** Graham enters scores on his phone at game night, and the content column is capped at `--content-width` (520px) — so there is **no** viewport where name + placement + three number boxes + TB fit on one line. Squeezed inline at 390px each box had ~34px of usable width and a typed two-digit score was **invisible** while the page scrolled sideways. A mixed row now **always** wraps onto two lines (name + placement, then `A + B = Total`) via a modifier class plus a zero-height `<span class="score-break">` flex line-break; the boxes are `flex: 0 1 4.5rem` with a `min-width` floor (never shrink below their content, never grow — growing pushed TB onto a third line), the row uses a tighter `column-gap`, spin buttons are removed (they reserved ~13px and clipped the "Switch" placeholder), and only the **placeholder** drops to 0.8rem — **inputs stay 1rem or iOS Safari auto-zooms on focus**. Deliberately **no width media query**: the constraint is the fixed content column, not the viewport. Everything is scoped to `.score-row--mixed` / `.photo-block`, so a **pure cup's row is untouched**. The mixed photo panel's three buttons (`.btn` is `white-space: nowrap` + `min-width: auto`) overflowed their card at every width, so `.photo-block .field-row` wraps and the third button is labelled just **"Read scores"**.
  - **Two photos, one per console** (issue #70). `cup_photos.block` (nullable INTEGER) tags which block a photo belongs to; **NULL = "the cup's photo"** (every pure cup, every pre-feature row — no backfill). Tagged by **ORDINAL, never an edition string**: the console is derived via `block_edition(cup, block)` = `race_edition(cup, (block-1)*RACES_PER_BLOCK + 1)`, exactly like races, so a photo row can never disagree with `first_edition`.
    - **`POST /cups/<id>/photo/<block>`** (JSON `{image, mime_type}` → `{ok, block, edition, label}`) persists one block's photo on **its own request**, rejecting `block ∉ (1,2)` (400) and any non-mixed / cancelled / deleted / missing cup via a `create_score`-style conditional `INSERT … WHERE EXISTS` (rowcount 0 → 404). **Why per-photo POSTs and not a form field:** `MAX_CONTENT_LENGTH` is 1 MB and two downscaled photos are ~0.55–1.1 MB of base64, so one form would intermittently **413 with a flash-less error page** — the silent-drop class this repo already fought. **Do not raise `MAX_CONTENT_LENGTH`.** It also lets the swap photo be taken during race 2 (long before the completion form exists) and means **the mixed completion form carries no `photo_data` at all**, removing the async attach/submit race for mixed cups (a test asserts the field is absent; the template adds a small all-empty submit check since `hasAnyScore()` no longer guards it).
    - **A mixed completion POST never stores a photo.** The mixed form carries no `photo_data`, but the completion route parsed it unconditionally, so a **crafted** POST could leave a mixed cup a `block = NULL` row: shadowed by the per-block thumbnails **and** unprunable (the retention `DELETE` compares `block = ?`, which never matches NULL), riding every Drive backup forever. `cup_session_submit` now short-circuits to `photo = None` on a mixed cup — a malformed smuggled payload can't bounce a valid submit either. The pure-cup form photo path is unchanged.
    - **`GET /cups/<id>/photo/<block>`** serves the newest photo for that block (same ETag/Cache-Control/304 as `cup_photo`). **`GET /cups/<id>/photo` is deliberately unchanged** — newest of ANY block; the `/cups` thumbnail and `test_newest_photo_wins_when_multiple` pin that. Replacing = another INSERT and newest-row-per-block wins, so a wrong-block upload is fixed by re-uploading rather than deleting. **RETENTION (added at the security gate): the block-upload route keeps exactly ONE row per `(cup_id, block)` — the newest — pruning the rest in the SAME transaction as the INSERT.** "Replace photo" is an ordinary action (blurry shot, wrong screen) and nothing ever serves an older row, so an append-only table would grow forever — and `scripts/backup.sh` snapshots the **whole DB** and pushes the **whole file** to Drive on every change, so each dead ~700 KB retake would be re-uploaded for the rest of time. The `DELETE` is scoped `cup_id = ? AND block = ?`, and `block = ?` is a non-NULL comparison, so **legacy blockless (whole-cup) rows can never be pruned** and other blocks/cups are untouched. (The pure-cup completion-form path is unchanged and still appends.) The response's `edition`/`label` come from a cup row **re-read if the pre-INSERT read missed** — otherwise the narrow interleaving "SELECT misses, conditional INSERT hits" turned a committed photo into a 500.
    - **Capture at the swap:** the race page renders a `#photo-block-1` panel when `show_block_photo_button` (mixed **and** `current_race` is `RACES_PER_BLOCK` or `+1` — race 2, where the screen is actually up, and race 3, where the reminder modal fires and needs the panel's hidden file inputs). It's labelled `finishing_console_label` — the console **actually finishing**, not the one coming up. `#swap-reminder-modal` carries the same control (its buttons are wired because `initBlockPhoto` queries them **document-wide by this block's input ids**), its dismiss button now reads **"Skip for now"**, taking a photo from inside it also dismisses it, and it keeps its `sessionStorage` one-shot. Skipping is always fine — that half just gets typed in.
    - **History/edit:** `/cups` and `/cups/<id>/edit` render **one thumbnail per block** for mixed cups, labelled with the real console (`block_photo_labels`); pure cups keep the single whole-cup thumbnail via the legacy route.
  - **Photo extraction is PER BLOCK, and each photo resolves exactly ONE base edition — its own console.** `_players_for_extraction(data, block)`: mixed + `block ∈ (1,2)` → `edition = block_edition(cup, block)` and **`partial_half = False`** (this photo IS the whole of that block, so it fills that block's field, not a cup total); mixed + **no** block → unchanged legacy behavior (`race_edition(cup, MAX_RACES)`, `partial_half = True`, `scores: {}`, `raw_rows` still returned so the read-only reference panel works); a `block` on a pure cup or the manual path → 400. `/extract-scores` also accepts `{cup_id, block}` with **no `image`** — it loads the newest stored photo for that block and base64s it server-side, which is how a swap photo becomes block-1 scores on the completion page (404 if none). The response gained `"block": n | null`.
    - **THE correctness rule of this feature.** `default_character_field(edition)` now **RAISES outside `BASE_EDITIONS`** (same posture as `courses_for('mixed')`); it used to return the **Switch** column for anything ≠ `'wii'`, including `'mixed'` and `None`. Measured on a real prod photo: on a **Switch** screen containing a **Baby Peach CPU at 39**, the matcher correctly gave Graham **Daisy 38** *because it read only `default_character_switch`* (Baby Peach is his **Wii** main). **If any code path ever resolves both editions' characters against one photo, or passes `'mixed'`, that trap returns and would auto-fill a CPU's score.** Worse on Switch specifically: the mk8dx prompt forces `is_highlighted=false` on every row, so Switch matching is character-only **with the off-character protection disabled** — it will hand a player a CPU's points the first time a CPU spawns on their main. The mitigations are the per-block mapping dropdown and freely editable block inputs: **never make a block value read-only, never auto-submit.** (Switch highlight detection is **issue #65** — a separate tradeoff, deliberately not solved here.)
    - **Extraction cannot detect a STALE photo.** Also measured: a real photo taken one race early read **perfectly** while being 10 and 15 points short of the truth. So "extraction succeeded" must never be presented as "these are the final scores" — the **visible per-block breakdown** (two numbers and their sum, each named for its console) is the guard. Digit reading itself is reliable (36/36 rows across 3 photos × 3 runs, including ties) — trust the values, distrust the framing. And **`position` can duplicate** (one run returned two rows numbered 8) — never key anything on it.
    - **Client:** `initBlockPhoto(opts)` in `static/js/photo_score.js` is a **SECOND entry point**, not a mode of `initPhotoScore` — that function owns the documented silent-drop guards and the shipped pure-cup flow and must not shift under this. Only `downscale` was hoisted to module scope (`downscalePhoto`) and shared. Each panel uploads → shows a **server-confirmed** "<Console> photo saved ✓" → extracts → fills `.block-score-input[data-block=N]` → renders its own mapping `<select>`s (highlighted-first, cross-dropdown disabling) writing into that block only. A shared `blockPhotoBusy` counter guards the single submit button across both panels.
    - **Privacy tradeoff, accepted deliberately:** a mixed cup sends **TWO photos to the Anthropic API — one per console — instead of one**, so twice as much living-room imagery leaves the box per cup. That is inherent to the feature (each console's screen has to be read on its own; reading one screen for both halves is the silently-wrong-number failure this exists to prevent). Photos are downscaled to ≤1200px JPEG before leaving the browser, extraction is optional (skip it and type the halves), and the whole thing is off without `ANTHROPIC_API_KEY`. Graham knows and accepts this.
    - **Pure cups are untouched end to end**: the `{% else %}` branch of `cup_session_complete.html` keeps the original `#photo-score` markup and `initPhotoScore({...})` call verbatim, with no block inputs anywhere.
  - **Both mixed-cup modals are one-shot per browser session** via `sessionStorage` (`km-console-flip-<id>`, `km-swap-reminder-<id>`) — their server-side gates are stateless (`no races yet` / `exactly 2 races`), so every refresh of that page would otherwise re-block the controls. The keys only suppress the *replay*; the outcome always comes from the DB, so clearing them can at worst re-show a modal, never change anything. The persistent `#second-console-banner` carries the swap message for the rest of the cup.
  ...
  - **Staging seed** (`scripts/seed_staging.py`) includes completed mixed cups in **both** console orders and parks the in-progress cup **exactly at the console swap**, so `break-staging` always has mixed data to hammer. Seeded mixed cups also carry a **per-block breakdown that adds up** (`split_blocks`) and a photo per block; the in-progress cup gets block 1's only (it's parked at the swap). Those photos are a **synthetic 1×1 JPEG** — this repo is public and a real standings photo is a picture of somebody's living room, so **never commit one** (tests and seeds use generated bytes). `.gitignore` now backs that up: **`*.jpg`/`*.jpeg`/`*.heic`/`*.heif` are ignored outright** (developing this feature means real photos sitting in the working tree as fixtures, two per cup, and a stray `git add .` in a public repo is a live risk — no JPEG has ever been tracked and the committed icons are `.png`/`.ico`). The `.env` rules were tightened the same way: **`.env*` with negations for `.env.example` and `.env.backup.example`**, so `.env.staging`/`.env.production` can't slip through a hand-maintained allowlist.
  - Tests: `tests/test_mixed_cups.py`, `tests/test_mixed_cup_block_photos.py`, `tests/e2e/test_mixed_cups.py`.
...
- **Route `POST /extract-scores`** (JSON): `{image: <base64>, mime_type: image/jpeg|png}` plus either `cup_id` (live session — players/edition come from the cup) or `edition` + `player_ids` (manual `/cups/new` form). Response `{scores: {player_id: points}, ambiguous: [names], unmatched_players: [names], raw_rows: [{position, character, points, is_highlighted}, ...]}` — each raw row carries `is_highlighted` so the client can order the mapping dropdowns (humans first). Hostile-input posture matches the rest of the app: bad base64/mime/oversize/ids → 4xx JSON, no key → 503, upstream API failure → 502, never a 500 (`tests/test_photo_extraction.py`).
  - **Per-IP throttle on the PAID call (added at the security gate — `EXTRACT_RATE_LIMIT_MAX` = 10 per `EXTRACT_RATE_LIMIT_WINDOW` = 5 min).** Every extraction is billed to Graham's Anthropic key and the app sits behind **one shared house password friends have**; there was previously **no limit of any kind** on this route (only the login gate and the JWKS refetch are limited). The stored-photo path removed even the natural brake of having to upload an image — a 22-byte `{"cup_id":1,"block":1}` makes the server ship ~1 MB to Anthropic, and the caller chose that photo's size (measured: 50 requests → 50 paid calls in 0.26 s). `_extract_rate_limited(_client_ip())` is the same lock+dict sliding window as the login limiter, **including the same caveats**: per-process, so N gunicorn workers means N× the ceiling (accepted — it converts an unbounded loop into a small constant), and `CF-Connecting-IP` is trusted only because the app is reachable solely through the tunnel. The check sits **immediately before `extract_standings`**, so a malformed request costs nothing and can't spend a real user's quota; a mixed cup (two photos + retries) is nowhere near the limit. Trip → **JSON 429** with an `error` sentence, which `photo_score.js` already renders verbatim next to its enter-by-hand fallback. **The bucket is module state shared by every test client, so `tests/conftest.py` clears `_extract_calls` per test** — don't remove that or unrelated tests will start 429ing. **Be honest about the real ceiling:** `entrypoint.sh` runs gunicorn with `--max-requests 1000 --max-requests-jitter 50`, so a worker recycle *drops the bucket entirely*. Against a determined authenticated caller the bound is therefore closer to "10 paid calls per ~1000 requests per worker" (~1% of requests billable) than a hard 10-per-5-minutes — a ~100× improvement on unbounded, not a real quota. This is the **same defect class as issue #68** (the login limiter is resettable the same way); fixing either properly needs shared state this app deliberately doesn't have. Don't quote the 10/5min figure as if it were a guarantee.
...
- **Photo persistence:** `cup_photos (id, cup_id, image BLOB, mime_type, created_at, block)` — stored as a BLOB **deliberately** so photos ride along with the existing rclone→Drive DB backups (no separate volume/backup story; photos are downscaled so growth is modest). Saved inside the same transaction as the scores in `create_cup` and `cup_session_submit`; saving never requires extraction to have run. A **malformed** `photo_data`/`photo_mime` rejects the whole submit with a flash. `GET /cups/<id>/photo` serves the newest blob with its stored mime (404 if none); `/cups` shows a thumbnail linking to it, and the **cup edit page** shows the photo full-width (linked to full-size; `edit_cup` passes `has_photo`). `MAX_CONTENT_LENGTH` was raised 256KB → **1MB** for the photo payloads — **do not raise it further**; the second photo a mixed cup needs gets its own request precisely so the body stays under it (see the mixed-cup section).
  - **`block` is NULL for everything described in this section** — it tags a MIXED cup's per-console photo (1/2) and NULL means "the cup's photo". A mixed cup uses a different flow entirely: **two photos, one per console**, each POSTed on its own to `/cups/<id>/photo/<block>`, extracted against **its own** console's edition, filling **its own** half — see **Mixed cups** above. The `partial_half` suppression documented there now applies only to a **blockless** mixed request (the legacy path); a request naming a block auto-fills normally, because the field it fills is that block's score, not the cup total.

## Line Finder (`/line-finder`)

A **read-only** analysis page answering one question for one hard-wired pair of players: *how many points should the second player get at the start of a cup so that it is a coin flip?* — per format (Wii cup, Switch cup, mixed 2 + 2), with the evidence behind the number (chart, per-line tables, margin trend, backtest, line history, the cups). No write path, no new dependencies, vanilla JS + inline SVG.

- **Routes:** `GET /line-finder` (page, `templates/line_finder.html`) and `GET /line-finder/data` (JSON the page renders from). Both responses carry `Cache-Control: no-store`. Query params: `half_life` (int 14…365, or `none`/`all` = unweighted; default 90), `two_player` (`0`/`1`, default 0 — keep only cups where `n_players == 2`), plus view-only `actual`/`fitted` (`0`/`1`, default 1) so a shared URL opens the same way. Empty values fall back to the default; anything else is a **400** (`parse_line_finder_params` raises `InvalidInput`), never a 500. Both routes are GET-only and sit behind the password gate like every page (not in `GATE_EXEMPT_PATHS`). Home has a "Line Finder" link.
- **Players:** `line_finder.DEFAULT_PLAYERS = ("Naked Graham", "EVDV")` — first = the favourite, second = who *receives* the line. Override with **`LINE_FINDER_PLAYERS="Name A,Name B"`** (read at request time; malformed → default). It exists only so **staging** can point at two seeded players — `docker-compose.staging.yml` sets `Test Toad,Dummy Diddy`, and `seed_staging.py` gives that pair cups on every console plus three head-to-head ones (`TWO_PLAYER_CUP_INDEXES`; seeded completed cups now also write `cup_players`). Prod needs no variable. If either name has no `players` row, the page shows a friendly empty state and the JSON returns `{"available": false, …}`.
- **Data (`line_finder.load_pair`):** cups with `status='completed' AND deleted_at IS NULL` where **both** players have a `scores` row. Margin = favourite's raw `scores.score` − receiver's raw `scores.score` (**never** `line_score`). `n_players` = `cup_players` count, falling back to the `scores` count for hand-entered cups (`POST /cups` writes no `cup_players`). Also reads both players' `scores.line` per cup (the receiver's is "the line actually used"; the backtest scores "used" as the **net** `b_line − a_line`, because the app decides winners on `line_score`), the receiver's `line_changes` **restricted to the pair's own completed cups** (a change from a cup the favourite wasn't in is never serialised), and `players.line` (stored line today). The JSON carries only what the page renders (cup ids only on `cups[]`, which links each row to `/cups/<id>/edit`).
- **Math (`line_finder.py`, pure, unit-tested):** cup weight `w = 0.5 ** (age_days / half_life)` (`None` → 1). Per edition, the recency-weighted **per-race edge** `mu = Σ w·m / Σ w·r` and unweighted per-race sd `sqrt(Σ (m − r·mu)² / Σ r)` are pooled over *samples*: a pure cup is `(margin, r=4)`, and each block of a **mixed cup with a per-console breakdown** is `(block margin, r=2)` credited to **its own console** (block 1 = `first_edition`, block 2 = the other; a mixed cup with NULL blocks still counts for the mixed actual curve but adds nothing here). Format models: Wii ~ N(4mu_w, (2sd_w)²), Switch likewise, Mixed ~ N(2mu_w + 2mu_s, 2sd_w² + 2sd_s²); fitted win % at line L = 1 − Φ((L − mean)/sd); sd needs ≥ 2 samples per edition, else "not enough cups". **Recommended line = model mean rounded half away from zero** (`recommend()` — deliberately NOT Python's banker's `round()`, which turns 12.5 into 12 and would read as a bug; don't "fix" it back) **— always the point estimate, even inside the noise** (Graham's call 2026-09-19, replacing an earlier rule that zeroed it and quietly favoured the stronger player on thin data; `recommend_line()` also returns a `within_noise` flag when `|mean| < SE`, which the tile shows as "inside the noise" plus a note that even the sign is uncertain — it never changes the line; a negative line renders on the tile as "<favourite> +N"). SE is the standard error of the cup-level mean (`sd_cup/√n`, the two-console analogue for mixed), shown beside the line together with, outside the noise, the nearest half-point ("use +8½ to rule out ties"). The raw fitted mean and SE stay in the JSON as they are; the one rule feeds the tiles, chart markers, shaded table rows and the backtest walk alike. **Actual win %** at L counts real cups of that format (a tie at L = half a win). **Pairs-proxy rule:** the mixed actual curve uses real mixed cups when ≥ 1 exists; otherwise every Wii cup × every Switch cup (`m = w/2 + s/2`), labelled "from Wii×Switch pairs" wherever it appears, with the proxy count mentioned in a note when real cups exist. Lines grid −10…35 in half steps (tables show the integers). **Backtest:** chronological walk, each cup recommended from only the cups before it (≥ 12 prior *races* — three cups' worth — of each console involved, counted in races so mixed blocks don't satisfy it three-for-three; else "not enough history"), tallied under the backtested line vs the net line actually used. Weights in the backtest are relative to each cup's own date (the weighted mean is scale-invariant, so this equals using today). A `cups.date` without a 4-digit year (a mangled row from the old unbounded `tz_offset`) is skipped and counted in `skipped_unparseable_dates`, never raised.
- **Sanity numbers** (the seeded staging pair, `Test Toad` vs `Dummy Diddy`, "all cups equal" — date-independent): Wii n=6, fitted +2.4 ± 7.2 → **even** (inside the noise); Switch n=2, +25.3 ± 9.3 → +25; Mixed n=2 real cups, +13.9 ± 5.9 → +14. Verified against prod data on 2026-09-18 (real figures deliberately not recorded here — this repo is public). The sd is the spec's population-style spread around the weighted mean, so it reads slightly tighter than a sample sd.
- **Page (`static/js/line_finder.js`, `.lf-*` styles in `app.css`):** controls (2-player, show actual/fitted, half-life slider whose far-right stop is "all cups equal") mirror the query string via `history.replaceState` and re-fetch the JSON (slider debounced 150 ms; actual/fitted toggles are client-side only). Everything is built with `createElement`/`textContent` — player names come from the DB. Series colors are the fixed `--lf-wii`/`--lf-switch`/`--lf-mixed` tokens (light + dark), deliberately not accent-themed. Tables scroll inside `.lf-table-wrap`; the page never scrolls sideways at 393 px (checked with Playwright). Static assets are referenced with plain `url_for('static', …)` like the rest of the app (no cache-busting scheme exists in this repo).
- **Tests:** `tests/test_line_finder.py` — weights, per-race pooling incl. mixed blocks, actual curve with ties, pairs proxy vs real-mixed switch-over, rounding incl. "even", not-enough-cups paths, backtest walk, 2-player filter, the DB loader's exclusions, route/JSON shape, param validation, empty state, gate, and a check that the seeded staging pair has cups on every console.
