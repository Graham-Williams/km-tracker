...
- **Mixed Cups (Wii + Switch)** — a 4-race cup split across both consoles in blocks: a coin flip at cup creation picks which console goes first, then it's 2 races there and 2 on the other. The wheel, the course validation, and the "already played" list all follow the current race's console. Because each console's scoreboard starts from zero, a mixed cup is scored **per console**: photograph the first console's results before you switch (there's a button on the last race of the first block, and a skippable reminder at the swap), photograph the second at the end, and each photo fills in **its own half** — the cup total adds itself up. Either half can be typed by hand instead; photos are always optional, and a saved photo can be replaced later. Mixed cups are lineless
